<div class="authors form content">
    <h1>Add author</h1>
    <?= $this->Form->create($author, ['type' => 'file', 'url' => $author->isNew() ? ['action' => 'add'] : ['action' => 'edit', $author->id]]) ?>
    <input type="file" class="dropify" name="image" data-default-file="<?= !empty($author->image) ? $this->url->image($author->image) : '' ?>" value="">
    ...
    <input type="text" class="form-control" id="rate" name="email" value="<?= h($author->email ?? '') ?>">

    <label for="status" class="form-label">Status</label>

    <label for="status" class="form-label">Active</label>
    <input type="radio" id="active" name="status" value="1" <?= (isset($author->status) && $author->status == 1) ? 'checked' : '' ?>>
    <label for="status" class="form-label">In Active</label>
    <input type="radio" id="notAvailable" name="status" value="0" <?= (isset($author->status) && $author->status == 0) ? 'checked' : '' ?>>
